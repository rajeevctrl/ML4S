...
trait NPTrait {
   ...
   */
  def max(arr1:INDArray,arr2:INDArray):INDArray={
    val arrMax = Transforms.max(arr1, arr2)
    return arrMax
  }
   ...
   */
  def min(arr1:INDArray,arr2:INDArray):INDArray={
    val arrMin = Transforms.min(arr1, arr2)
    return arrMin
  }
  
  /**
   * Sum of elements of given {@code arr} along a given {@code axis}. 
   * If {@code axis} is not specified then all elements are added.
   */
  def sum(arr:INDArray,axis:Array[Int]=Array.ofDim(0)):INDArray={
    val axis_ = if(axis.size>0){
      axis
    }else{
      Range(0,arr.shape().size).toArray
    }
    return arr.sum(axis: _*)
  }
